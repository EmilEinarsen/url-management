...
const dataToQueryString = (data) => {
    let strings = [];
    Object.keys(data).forEach((key) => strings.push(encodeURIComponent(key) + '=' + JSON.stringify(data[key])));
    return strings.join("&");
};
...
const createUrl = (params) => window.location.origin
    + window.location.pathname
    + '?'
    + params;
const removeEmpty = (obj) => (Object.entries(obj).forEach(([key, val]) => val && typeof val === 'object'
    ? !(Object.keys(val)?.length ?? val.length) ? delete obj[key] : removeEmpty(val)
    : val == null && delete obj[key]), obj);
const setObjToUrl = (data) => {
    let updatedData = { ...getDataFromUrl(), ...data };
    const url = createUrl(dataToQueryString(removeEmpty(updatedData)));
    window.history.pushState({ path: url }, '', url);
};
const getButSetIfEmpty = ({ key, defaultValue }) => getDataFromUrl()[key] ?? (setObjToUrl({ [key]: defaultValue }),
    getDataFromUrl()[key]);
